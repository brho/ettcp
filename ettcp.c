...
char **argv;
{
	...
	while ((c = getopt(argc, argv, "cdrstuvBDTqb:f:l:n:p:A:O:i:")) != -1) {
		...
		case 'D':
			...
			nodelay = 1;
#else
			fprintf(stderr, "ttcp: -D option ignored: TCP_NODELAY "
...
#else
			fprintf(stderr, "ttcp: -b option ignored: "
	...
	if (bind(fd, SA(&sinme), sizeof(sinme)) < 0)
		err("bind");
	...
	if (!udp) {
		...
		if (trans || c_flag) {
			...
			if (nodelay) {
				...
				if (p &&
				    setsockopt(fd, p->p_proto, TCP_NODELAY,
				               &one, sizeof(one)) < 0)
					err("setsockopt: nodelay");
				mes("nodelay");
			}
#endif
			if (connect(fd, SA(&sinhim), sizeof(sinhim)) < 0)
				err("connect");
			if (!q_flag)
				mes("connect");
		} else {
 ...
 */
			...
			domain = AF_INET;
			if ((fd = accept(fd, SA(&frominet), &fromlen)) < 0)
				err("accept");
			{
				struct sockaddr_in peer;
				int peerlen = sizeof(peer);
				if (getpeername(fd, SA(&peer), &peerlen) < 0) {
					err("getpeername");
				}
				if (!q_flag)
					fprintf(stderr,
	...
	if (sinkmode) {
		...
		if (trans) {
			...
			if (0 == interval) {
				...
				while (Nwrite(fd, buf, buflen) == buflen) {
					nbytes += buflen;
...
void err(s) char *s;
{
	fprintf(stderr, "nttcp%s: ", trans ? "-t" : "-r");
	perror(s);
...
pattern(cp, cnt) register char *cp;
register int cnt;
{
	register char c;
	c = 0;
...
#if defined(SYSV)
/*ARGSUSED*/
static getrusage(ignored, ru) int ignored;
...
/*ARGSUSED*/
static gettimeofday(tp, zp) struct timeval *tp;
struct timezone *zp;
{
	tp->tv_sec = time(0);
	tp->tv_usec = 0;
...
int count;
{
	...
	register int cnt;
	if (udp) {
		cnt = recvfrom(fd, buf, count, 0, SA(&from), &len);
		numCalls++;
	} else {
		if (b_flag)
...
int count;
{
	...
	if (udp) {
	again:
		cnt = sendto(fd, buf, count, 0, SA(&sinhim), sizeof(sinhim));
		numCalls++;
